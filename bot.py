...
base_queue = list(BASE_PRESCRIPTIONS)
random.shuffle(base_queue)

simple_queue = list(SIMPLE_PRESCRIPTIONS)
random.shuffle(simple_queue)
...
async def wait_until_morning():
    ...
    while True:
        ...
        if now.hour >= 20 or now.hour < 8:
            ...
            if now.hour >= 20: wait_seconds += 86400
...
async def march_trigger(post: types.Message):
    ...
    start_index = text_lower.find("март")
    quote_text = post.text[start_index:start_index+4]
    
    simple_task = get_next_simple()
    caption = f"_ПРЕДПИСАНИЕ ПОЛУЧЕНО._\n\n{simple_task}"
    
    try:
        reply_params = ReplyParameters(message_id=post.message_id, quote=quote_text)
        if STICKER_ID:
            await bot.send_sticker(chat_id=CHANNEL_ID, sticker=STICKER_ID, reply_parameters=reply_params)
...
async def main():
    if not API_TOKEN or not CHANNEL_ID: return
...
async def march_trigger(post: types.Message):
    ...
    simple_task = random.choice(SIMPLE_PRESCRIPTIONS)
    caption = f"_ПРЕДПИСАНИЕ ПОЛУЧЕНО._\n\n{simple_task}"
...
async def march_trigger(post: types.Message):
    ...
    try:
        photo = FSInputFile(IMAGE_PATH)
        reply_params = ReplyParameters(message_id=post.message_id, quote=quote_text)
        await bot.send_photo(chat_id=CHANNEL_ID, photo=photo, caption=caption, reply_parameters=reply_params)
        waiting_for_execution = True
    except Exception as e:
        print(f"Error: {e}")
...
async def wait_until_morning():
    ...
    while True:
        ...
        if now.hour >= 20 or now.hour < 8:
            if now.hour >= 20:
                target_time = now.replace(hour=8, minute=0, second=0, microsecond=0)
                wait_seconds = (target_time - now).total_seconds() + 86400
            else:
                target_time = now.replace(hour=8, minute=0, second=0, microsecond=0)
                wait_seconds = (target_time - now).total_seconds()
            await asyncio.sleep(min(wait_seconds, 3600))
